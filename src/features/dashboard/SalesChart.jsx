...
function SalesChart({ bookings = [], numDays }) {
  ...
  const data = allDates.map((date) => {
    const { totalSales, extrasSales } = bookings.reduce(
      (acc, booking) => {
        const createdAt =
        ...
        if (!isSameDay(date, createdAt)) return acc;

        const totalPrice = Number(booking.totalPrice ?? 0);
        const extrasPrice =
          booking.extrasPrice ??
          (booking.hasBreakfast
            ? Number(booking.numNights ?? 0) * 15 * Number(booking.numGuests ?? 0)
            : 0);

        acc.totalSales += totalPrice;
        acc.extrasSales += Number(extrasPrice ?? 0);
        return acc;
      },
